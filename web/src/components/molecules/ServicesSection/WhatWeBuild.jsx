...
import { webAppDevContent } from "@/content";

const { title, label, heading } = webAppDevContent;

export default function WhatWeBuild() {
  const { status, body, cols } = useServiceContent(title);

  return (
    <FlowSection aria-label={label} style={{ backgroundColor: colors.bgBrand, color: colors.white }}>
